...
## Phase 7.4 — Signing keys and the updater plumbing (LANDED 2026-09-12)

Key generated by Eric at `~/.tauri/richochet-updater.key`; 7.4.0–7.4.5 are in. **7.4.6 is still
outstanding** — the two Actions secrets have to be set in GitHub repo settings by a human, and
nothing signs on CI until they are.

One consequence worth knowing, and it is the reason 7.4.0 was deferred this long: with
`createUpdaterArtifacts: true` the bundler signs, so **`just build` now needs the key**. Locally:

```powershell
$env:TAURI_SIGNING_PRIVATE_KEY = "$HOME\.tauri\richochet-updater.key"   # a path is accepted
ichochet-updater.key"   # a path is accepted
$env:TAURI_SIGNING_PRIVATE_KEY_PASSWORD = "<passphrase>"
just build
```

`release-dry-run.yml` was updated to pass the same two secrets and to upload the `.sig` alongside
the `.exe` — the signature is the half the updater actually verifies, and its absence is the
symptom of signing having silently not happened.
